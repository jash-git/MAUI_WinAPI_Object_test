...
function Main() {
    ...
    */
    ...
    //---
    //新增列印主體內容
    ESC_Value.push(ecTEXT_ALIGN_CENTER + ecBOLD_ON + ecBIG_ON + json_obj.company_shortname + ecBIG_OFF + ecBOLD_OFF+ ecFREE_LINE + ecFREE_LINE);//文字至中 + 粗體+放大 + 店名 + 換行
    ...
    ESC_Value.push(ecTEXT_ALIGN_LEFT + strbuf + ecFREE_LINE);//文字靠左 + 日期(時間) + 換行
    
    strbuf = '交易序號: ' + json_obj.order_no;
    ESC_Value.push(ecTEXT_ALIGN_LEFT + strbuf + ecFREE_LINE);//文字靠左 + 交易序號 + 換行
    ...
    if (json_obj.order_items != null) {
        for (var i = 0; i < json_obj.order_items.length; i++) {
            ...
            if ((json_obj.order_items[i].product_type == 'P') || (json_obj.order_items[i].product_type == 'K')) {//一般餐點和包材
                ...
                if ((json_obj.order_items[i].set_meals != null) && (json_obj.order_items[i].set_meals.length > 0)) {
                    for (var j = 0; j < json_obj.order_items[i].set_meals.length; j++) {
                        ...
                        var count = "" + json_obj.order_items[i].set_meals[j].count;
                        spaceCount = 6 - Wlen(count) - 2;
                        for (var k = 0; k < spaceCount; k++) {
                            space += " ";
                        }
                        count = "X" + space + json_obj.order_items[i].set_meals[j].count;
    ...
    ESC_Value.push(ecTEXT_ALIGN_LEFT + ecBIG_ON + strbuf + ecBIG_OFF + ecFREE_LINE);//文字靠左 + 放大 + 總計數量 + 換行
