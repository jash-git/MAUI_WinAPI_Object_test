...
function Main() {
	...
	strbuf = strbuf + ' ~ ' + year + "-" + month + "-" + day + " " + hour + ':' + minute;//strbuf = ShiftSpace + '          ' + year + "-" + month + "-" + day + " " + hour + ':' + minute;
	ESC_Value.push(ecTEXT_ALIGN_LEFT + strbuf + ecFREE_LINE);
	...
	if( (json_obj.category_sale_info!=null) && (json_obj.category_sale_info.length>0) )
	{
		...
		{
			var intSpaceLen = 28 - Wlen(json_obj.category_sale_info[i].category_name) - 1;
			...
			var intSpaceLen = 13 - Wlen(''+json_obj.category_sale_info[i].subtotal);
