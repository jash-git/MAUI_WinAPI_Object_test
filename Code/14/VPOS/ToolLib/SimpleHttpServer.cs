...
namespace VPOS
{
        ...
        ****************************************************************************/
        ...
        public static bool HttpBodyCreate(String StrType, String StrPath, String StrInput, ref String StrResult)
        {
            ...
            if((StrType.Length>0) && (StrPath.Length>0))
            {
                switch(StrPath)
                {
                    case "/getorderno"://訂單取號
                        blnResult = HttpAPI.getorderno(StrType, ref StrResult);
                        break;
                    case "/getdailyno"://關帳取號
                        blnResult = HttpAPI.getdailyno(StrType, ref StrResult);
                        break;
                    case "/getclassno"://交班取號
                        blnResult = HttpAPI.getclassno(StrType, ref StrResult);
                        break;
                    case "/testconnection"://連線測試
                        blnResult = HttpAPI.testconnection(StrType, ref StrResult);
                        break;
                    default:
                        blnResult = false;
                        break;
    ...
    public class HttpAPI
    {
        ...
        public static bool getorderno(String StrType, ref String StrResult)
        {
            bool blnResult = false;
            if (StrType == "get")
            {
                String StrGuid = Guid.NewGuid().ToString().Replace("-", "").ToUpper();
                String StrOrderNo = MainPage.SerialCodeDataGet(StrGuid, 0);//取號函數,0為訂單號碼
                blnResult = (StrOrderNo.Length > 0);
                StrResult = StringToUnicode(String.Format("{{\"orderno\":\"{0}\",\"systime\":\"{1}\"}}", StrOrderNo, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")));
            }
            return blnResult;
        }

        public static bool getdailyno(String StrType, ref String StrResult)
        {
            bool blnResult = false;
            if (StrType == "get")
            {
                String StrGuid = Guid.NewGuid().ToString().Replace("-", "").ToUpper();
                String StrDailyNo = MainPage.SerialCodeDataGet(StrGuid, 1);//取號函數,1為關帳號
                blnResult = (StrDailyNo.Length > 0);
                StrResult = StringToUnicode(String.Format("{{\"dailyno\":\"{0}\",\"systime\":\"{1}\"}}", StrDailyNo, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")));
            }
            return blnResult;
        }

        public static bool getclassno(String StrType, ref String StrResult)
        {
            bool blnResult = false;
            if (StrType == "get")
            {
                String StrGuid = Guid.NewGuid().ToString().Replace("-", "").ToUpper();
                String StrClassNo = MainPage.SerialCodeDataGet(StrGuid, 2);//取號函數,2為交班碼
                blnResult = (StrClassNo.Length > 0);
                StrResult = StringToUnicode(String.Format("{{\"classno\":\"{0}\",\"systime\":\"{1}\"}}", StrClassNo, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")));
            }
            return blnResult;
        }

        public static bool testconnection(String StrType, ref String StrResult)
        {
            bool blnResult = false;
            if (StrType == "get")
            {
                blnResult = true;
                StrResult = StringToUnicode(String.Format("{{\"servername\":\"{0}\",\"systime\":\"{1}\"}}","vpos", DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")));
            }
            return blnResult;
        }
    }
