...
namespace VPOS
{
    public class FoodpandaAPI
    {
        ...
        public static Foodpanda_ordersnew m_Foodpanda_ordersnew = new Foodpanda_ordersnew();
        ...
        public static bool ordersnew()////抓取資料&寫進DB
        {
            ...
            if (Authentication())
            {
                ...
                String StrOutput = HttpsFun.RESTfulAPI_get(StrDomain, $"/api/takeaways/foodpanda/orders/new", "", StrHeaderName, StrHeaderValue);//String StrOutput = HttpsFun.RESTfulAPI_get(StrDomain, $"/api/vtstore/orders/new", "", StrHeaderName, StrHeaderValue);
                m_Foodpanda_ordersnew = JsonClassConvert.Foodpanda_ordersnew2Class(StrOutput);
                if ((m_Foodpanda_ordersnew != null) && (m_Foodpanda_ordersnew.status == "OK"))
                {
                    String SQL = "";
                    //---
                    //清空還未處理的訂單
                    SQL = "DELETE FROM takeaways_order_data WHERE platform_sid='foodpanda' AND (order_state = 'CREATED' OR order_state = 'ACCEPTED')";
                    SQLDataTableModel.SQLiteInsertUpdateDelete("Takeaways", SQL);
                    //---清空還未處理的訂單

                    //---
                    //新增所有新訂單
                    for (int i = 0; i < m_Foodpanda_ordersnew.data.Count; i++)
                    {
                        String SID = SQLDataTableModel.UUID2SID(64);//UUID
                        String platform_sid = "foodpanda";
                        String external_order_id = m_Foodpanda_ordersnew.data[i].external_order_id;
                        String order_no = m_Foodpanda_ordersnew.data[i].order_no;
                        double order_time = m_Foodpanda_ordersnew.data[i].order_time;
                        String order_type = m_Foodpanda_ordersnew.data[i].order_type;
                        String order_state = m_Foodpanda_ordersnew.data[i].order_state;
                        String call_num = (m_Foodpanda_ordersnew.data[i].call_num != null) ? m_Foodpanda_ordersnew.data[i].call_num : "";
                        int item_count = m_Foodpanda_ordersnew.data[i].item_count;
                        int subtotal = m_Foodpanda_ordersnew.data[i].subtotal;
                        int promotion_fee = m_Foodpanda_ordersnew.data[i].promotion_fee;
                        int discount_fee = m_Foodpanda_ordersnew.data[i].delivery.delivery_fee;
                        int delivery_fee = m_Foodpanda_ordersnew.data[i].delivery.delivery_fee;
                        int service_fee = 0;//m_Foodpanda_ordersnew.data[i].service_fee;
                        int amount = m_Foodpanda_ordersnew.data[i].amount;
                        String payment_type = (m_Foodpanda_ordersnew.data[i].payment_type != null) ? m_Foodpanda_ordersnew.data[i].payment_type : "";
                        String cust_name = m_Foodpanda_ordersnew.data[i].customer.first_name+ m_Foodpanda_ordersnew.data[i].customer.last_name;//(m_VTSTORE_ordersnew.data[i].customer.full_name != null) ? m_VTSTORE_ordersnew.data[i].customer.full_name : "";
                        String cust_phone = m_Foodpanda_ordersnew.data[i].customer.phone;//(m_VTSTORE_ordersnew.data[i].customer.mobile_phone != null) ? m_VTSTORE_ordersnew.data[i].customer.mobile_phone : "";
                        String cust_tax_number = "";//(m_VTSTORE_ordersnew.data[i].customer.tax_number != null) ? m_VTSTORE_ordersnew.data[i].customer.tax_number : "";
                        double cust_reserve_time = 0;//m_Foodpanda_ordersnew.data[i].cust_reserve_time;
                        ...
                        String remarks = (m_Foodpanda_ordersnew.data[i].remarks != null) ? m_Foodpanda_ordersnew.data[i].remarks : "";
                        double estimated_read_time = 0;//預計完成時間
                        String original_data = JsonClassConvert.FPAON_Datum2String(m_Foodpanda_ordersnew.data[i]);
                        String sync_state = "N";
                        String sync_msg = "";
                        String pos_order_no = "";
            ...
            */
            ...
            if (Authentication())
            {
                ...
                */
                ...
                String StrOutput = HttpsFun.RESTfulAPI_postBody(StrDomain, $"/api/takeaways/foodpanda/orders/{Strexternal_order_id}/action/{Straction_state}", StrInput, StrHeaderName, StrHeaderValue);//String StrOutput = HttpsFun.RESTfulAPI_postBody(StrDomain, $"/api/vtstore/orders/change_state", StrInputName, StrInputValue, StrHeaderName, StrHeaderValue);
